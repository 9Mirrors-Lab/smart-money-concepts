...
export function Engine2DiagnosticsContent() {
    ...
    <main className="w-full max-w-5xl space-y-8 px-4 py-6">
      <Card className="border-border">
        ...
        <CardContent className="space-y-3">
          <p className="text-sm">
            Logic applied site-wide: <span className="font-semibold">{versionLabel}</span>
            {" · "}
            <Link href="/engine2?tab=tune" className="text-primary underline">
              Change in Tune
            </Link>
          </p>
          ...
          {flowData && (
            <p className="text-sm text-muted-foreground">
              Showing loaded data for {loadSymbol.trim() || DEFAULT_LOAD_SYMBOL} (current version).
            </p>
          )}
          {activeConfig && (
            <div className="rounded-md border border-border bg-muted/30 font-mono text-xs">
              <p className="mb-1.5 px-2 pt-2 text-muted-foreground">
                Active thresholds (alignment → confidence → bias)
              </p>
                ...
                <tbody>
                  {ENGINE2_LOGIC_ENTRIES.filter(
                    ...
                    <tr key={entry.key} className="border-t border-border/50">
                      <td className="py-1 pl-2 pr-3 text-muted-foreground">{entry.key}</td>
      ...
      <div className="pb-8 text-sm text-muted-foreground">
        ...
        W/M/D/S = Weak, Moderate, Disaligned, Strong. N/C/E = Neutral, Continuation, Exhaustion. Bold =
        dominant; red + icon = unreachable.
      </div>
    </main>
  );
}
