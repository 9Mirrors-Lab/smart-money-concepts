#!/usr/bin/env node
/**
 * generate-components-plugin.mjs
 *
 * Generates a Figma plugin that creates ACTUAL editable Figma components
 * (Auto Layout frames, text layers, fills, borders, etc.) — NOT screenshots.
 *
 * Run:  node generate-components-plugin.mjs
 * Out:  figma-components-plugin/manifest.json + code.js + ui.html
 */

import { writeFileSync, mkdirSync } from "node:fs";

mkdirSync("figma-components-plugin", { recursive: true });

// ─── manifest.json ──────────────────────────────────────────────────────────
writeFileSync("figma-components-plugin/manifest.json", JSON.stringify({
  name: "SMC Viewer — Build Components",
  id: "smc-viewer-build-components",
  api: "1.0.0",
  main: "code.js",
  ui: "ui.html",
  editorType: ["figma"],
...
// ─── ui.html ────────────────────────────────────────────────────────────────
writeFileSync("figma-components-plugin/ui.html", `<!DOCTYPE html>
<html>
<head>
  <style>
    * { box-sizing: border-box; }
    body { font-family: Inter, sans-serif; padding: 20px; background: #1c1c1e; color: #e0e0e0; margin: 0; }
    h2 { margin: 0 0 6px; font-size: 15px; font-weight: 600; }
    p { font-size: 12px; color: #888; margin: 0 0 16px; line-height: 1.5; }
    button {
      background: #18a0fb; color: #fff; border: none; border-radius: 6px;
      padding: 10px 0; font-size: 13px; cursor: pointer; width: 100%; font-weight: 500;
    }
    button:hover { background: #0d8de8; }
    button:disabled { background: #444; cursor: default; }
    #status { margin-top: 12px; font-size: 12px; color: #aaa; min-height: 18px; }
    #progress { margin-top: 8px; height: 4px; background: #333; border-radius: 2px; overflow: hidden; }
    #progress-bar { height: 100%; width: 0%; background: #18a0fb; transition: width 0.3s; border-radius: 2px; }
  </style>
</head>
<body>
  <h2>SMC Viewer — Build Components</h2>
  <p>Creates 6 fully editable Figma screens with Auto Layout frames, text layers, and styled components matching the SMC Viewer app design system.</p>
  <button id="run">Build All Screens</button>
  ...
  <script>
    ...
    window.onmessage = function(e) {
      ...
      if (msg.type === 'progress') {
        ...
        document.getElementById('status').textContent = '✅ Done! All screens created.';
...
// ─── code.js ────────────────────────────────────────────────────────────────
// Color tokens from globals.css (dark mode) — OKLCH values converted to sRGB hex
// oklch(L C H) with C=0 → achromatic, just use L as grey
// oklch(0.145 0 0) → #212121, oklch(0.985 0 0) → #f9f9f9, etc.
const codeJs = `// SMC Viewer — Build Components Plugin
...
writeFileSync("figma-components-plugin/code.js", codeJs);

console.log("✅ Figma component plugin generated in ./figma-components-plugin/");
console.log("   manifest.json, code.js (" + Math.round(codeJs.length / 1024) + " KB), ui.html");
